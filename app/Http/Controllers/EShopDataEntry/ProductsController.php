...
class ProductsController extends Controller
{
    ...
    {
        //optimize image
    	if(array_key_exists('filepath', $_POST) && count($_POST['filepath']) > 0 && mb_strlen($_POST['filepath'][0]) > 0)      
        ...
        $checkAlias = AliasProduct::find($where['id']);
        if(count($checkAlias) > 0) AliasProduct::destroy($where['id']);
        $aliasProd = new AliasProduct;
        $aliasProd->id = (int)$where['id'];
        $aliasProd->alias = $where['alias'];
        $aliasProd->save();
    ...
    {
        $basePath = base_path() . '/public/image/';
